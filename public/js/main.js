...
(function (undefined) {
    ...
    var args = {
        ...
        resultFunction: function (res) {
            ...
            if ($('#scanned-QR1').val(res.code)) {
                var kode = $('#scanned-QR1').val();
                $.ajax({
                    url: "verifysertifikat",
                    method: 'get',
                    data: 'verify=' + kode,
                    dataType: 'json',
                    success: function (data) {
                        if (data.verify == kode) {
                            alert('Congratulation, Sertifikat terdaftar dengan nomor : ' + data.kode);
                        } else {
                            alert('Failed !!!!, Data tidak sesuai!!!');
