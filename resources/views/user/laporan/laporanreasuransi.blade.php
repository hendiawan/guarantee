                ...
                <table border="1"  style=" font-family: Arial, Helvetica, sans-serif; font-size: 10px; border-collapse: collapse;"  class="table table-hover"  >
                        <thead>
                            <tr style="background-color:#e0e0e0 ;color: #000000">
                                ...
                                <th style="width: 10px">Total (Hari)</th>                                
                                <th style="width: 10px">sd Akhir (Hari)</th>                                
                                <th style="width: 10px">Periode (Hari)</th>                                
                                <th>PLAFON (Rp)</th>
                                <th>PENJAMINAN (%)</th>
                                ...
                                <th>BEBAN REASURANSI PERHARI(Rp)</th>
                                <th>BEBAN REASURANSI PERIODE(Rp)</th>
                        ...
                        <tbody>
                            ...
                            <tr style="background-color:whitesmoke ;color: #000000">  
                                ...
                                <td>{{$cekJumlahTotal              =$akrual->cekJumlahTotal($datas->tglrealisasi,$datas->tgljatuhtempo)}}</td>     
                                <td>{{$HitungSdAkhir                =$akrual->HitungSdAkhir($datas->tgl_proses,$datas->tglrealisasi,$datas->tgljatuhtempo,$dari,$sampai,null)}}</td>     
                                <td>{{$HitungJumlahPeriode  =$akrual->HitungJumlahPeriode($datas->tgl_proses,$datas->tglrealisasi,$datas->tgljatuhtempo,$dari,$sampai,null)}}</td>     
                                <td  align="right">{{number_format( $datas->plafon, 0, ',', '.')}}</td>      
                                <td  align="right">{{number_format( $share = $datas->penjaminan->bank->share, 0, ',', '.')}}</td>      
                                ...
                                <td align="right">{{number_format( $datas->share_risk, 10, ',', '.')}}</td>  
                                ...
                                @endif 
                                <td align="right">{{number_format( $BebanReasPerhari=$nettijpreas/$cekJumlahTotal, 0, ',', '.')}}</td>  
                                <td align="right">{{number_format( $BebanReasPeriode=$BebanReasPerhari*$HitungJumlahPeriode, 0, ',', '.')}}</td>  
                            ...
                            $totalijpReasuransi = $totalijpReasuransi + $ijpreas;
                            ...
                            $totalBebanReasPeriode= $totalBebanReasPeriode + $BebanReasPeriode;

                            $i++
                            ?>
                            @endforeach
                            <tr style="background-color:#e0e0e0 ;color: #000000">
                                <td align='center' colspan="14">TOTAL</td>
                                ...
                                <td align='right'><b> </b></td>
                                <td align='right'><b>{{number_format( $totalijpReasuransi, 0, ',', '.')}}</b></td> 
                                 <td align='right'><b>{{number_format( $totalricommision, 0, ',', '.')}}</b></td>
                                <td align='right'><b>{{number_format( $totalnettijpreas, 0, ',', '.')}}</b></td>
                                <td align='right'><b> </b></td>
                                <td align='right'><b> </b></td>
                                <td align='right'><b>{{number_format( $totalBebanReasPeriode, 0, ',', '.')}}</b></td>
                                <td align='right'></td> 
                            </tr>
                        </tbody>
                    </table> 
