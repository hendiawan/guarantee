...
<div style=" width: 800px;
          ...
          <div style="border-radius: 20px;" class="panel panel-default">
            ...
            <div  style=" border-top-style: solid;
                ...
                <div class="panel-body">                   
                    <form method="POST" id="formReasuransi" action="{{url('rekanan/update-reasuransi')}}" enctype="multipart/form-data">
                        {{csrf_field()}}
  ...
  value="{{$datareasuransi->kodesertifikat}}"
  required="" id="no_sertifikat"  name="no_sertifikat">
                             <br>
                             ...
                             <div id="table-edit"class="panel panel-default ">
                                 <table class="table table-dark">
                                     <thead>
                                         <tr> 
                                             <th scope="col">Nama Terjamin</th>
                                             <th scope="col"> {{$datareasuransi->nama}}</th>
                                     ...
                                     <tbody>
                                         <tr> 
                                             <td>Bank</td>
                                             <td>{{$datareasuransi->namabank}}</td> 
                                         </tr>
                                         <tr> 
                                             <td>Plafon</td>
                                             <td>{{number_format($datareasuransi->plafon, 0, ',', '.')}}</td> 
                                         </tr>
                                         <tr> 
                                             <td>Periode Pertanggungan</td>
                                             <td>{{$datareasuransi->masakredit .' Bulan, dari :  '. date('d-m-Y', strtotime($datareasuransi->tglrealisasi)).'/'.date('d-m-Y', strtotime($datareasuransi->tgljatuhtempo))}}</td> 
                                         </tr>
                                     </tbody>
                                 </table>           
                             </div>     
                        ...
                        <div class="form-group">
                            ...
                            <select  id="pilihReasuransi" required="" name="id_reasuransi" class="form-control">
                                ...
                                <option     @if($datareasuransi->rekanan_id==$data->id) selected=""  @endif   value="{{$data->id}}">{{$data->nama_asuransi}}</option> 
                                @endforeach 
                            </select>    
                        </div>
                            <div class="form-group">
                                <label>R/I Commision<b style="color: red">( * )</b></label>  
                                <input value="{{$datareasuransi->commision}}"    style="border-radius: 30px;" required="" name="commision" id="poster" type="input" class="form-control"><br/>
                            ...
                            <div class="form-group">
                                ...
                                <input value="{{$datareasuransi->share_risk}}" style="border-radius: 30px;" required=""   class="form-control" name="share">
                            </div>
                        
                              <input hidden="" value="{{$datareasuransi->t_reasuransi_id}}"  name="t_reasuransi_id" id="t_reasuransi_id" type="input" class="form-control"><br/>
                              <input hidden="" value="{{$datareasuransi->penjaminan_id}}"  name="penjaminan_id" id="penjaminan_id" type="input" class="form-control"><br/>
                              <input hidden="" value="{{$datareasuransi->premi}}"  name="ijp" id="ijp" type="input" class="form-control"><br/>
                              <input hidden="" value="{{$datareasuransi->plafon}}"   name="plafon" id="plafonSimpan" type="input" class="form-control"><br/>
